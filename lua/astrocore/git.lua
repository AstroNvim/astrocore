--- ### Git LUA API
--
-- This module can be loaded with `local git = require "astrocore.git"`
--
...
function git.cmd(args, ...)
  ...
  return utils.cmd(vim.list_extend({ "git", "-C", astronvim.install.home }, args), ...)
end

--- Get the first worktree that a file belongs to
---@param file string? the file to check, defaults to the current file
---@param worktrees table<string, string>[]? an array like table of worktrees with entries `toplevel` and `gitdir`, default retrieves from `features.git_worktrees` from `setup()`
...
function git.parse_remote_url(str)
  return vim.fn.match(str, utils.url_matcher) == -1
