...
function M.setup(opts)
  ...
  cmd("AstroChangelog", function() require("astrocore.updater").changelog() end, { desc = "Check AstroNvim Changelog" })
  cmd(
    "AstroUpdatePackages",
    function() require("astrocore.updater").update_packages() end,
    { desc = "Update Plugins and Mason" }
  )
  cmd("AstroRollback", function() require("astrocore.updater").rollback() end, { desc = "Rollback AstroNvim" })
  ...
  if type(M.config.polish) == "function" then pcall(M.config.polish) end
end
