...
T["AC-ENV-011 restores package entries and discards queued callbacks after failure"] = function()
  ...
  local callback_failed = pcall(function()
    helpers.with_module(module_name, {}, function(module, context)
      MiniTest.expect.equality(module.value, "fixture")
      vim.schedule(function() vim.g.astrocore_unit_helper_scheduled = true end)
      vim.defer_fn(function() vim.g.astrocore_unit_helper_deferred = true end, 10)
  ...
  vim.wait(20)
  MiniTest.expect.equality(vim.g.astrocore_unit_helper_scheduled, nil)
  MiniTest.expect.equality(vim.g.astrocore_unit_helper_deferred, nil)
  package.preload[module_name] = original_preload
end

T["AC-ENV-012 restores nested vim fields after callback failure"] = function()
